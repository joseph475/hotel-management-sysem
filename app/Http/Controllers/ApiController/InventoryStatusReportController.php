...
class InventoryStatusReportController extends Controller
{
    public function printperRoom($id = ''){
        ...
        if($id != ''){
            ...
            FROM roominventory A inner join inventory_category B on A.inventory_id = B.id where room_id = '. $id .' GROUP by room_id, inventory_id, B.description');
        }
        else{
            $inventoryStatus = DB::select('select room_id, B.description, 
            ...
            FROM roominventory A inner join inventory_category B on A.inventory_id = B.id GROUP by room_id, inventory_id, B.description');
        }
